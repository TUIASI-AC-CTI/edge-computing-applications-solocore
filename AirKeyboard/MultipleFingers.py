...
active_keys = {finger: None for finger in FINGER_NAMES}
key_start_time = {finger: 0 for finger in FINGER_NAMES}
key_last_seen = {finger: 0 for finger in FINGER_NAMES}
...
while True:
    ...
    finger_keys_detected = {finger: None for finger in FINGER_NAMES}
                ...
                if dist1 < 60 and dist2 < 60 and dist3 < 80:
                    for key, (x1, y1, x2, y2) in key_positions.items():
                        if x1 < tip_pos[0] < x2 and y1 < tip_pos[1] < y2:
                            finger_keys_detected[finger] = key
                            key_last_seen[finger] = current_time
                            cv2.rectangle(frame, (x1, y1), (x2, y2), FINGER_COLORS[finger], 3)
                            break
                    cv2.circle(frame, tip_pos, 10, FINGER_COLORS[finger], -1)

    for finger in FINGER_NAMES:
        detected_key = finger_keys_detected[finger]
        if active_keys[finger]:
            if detected_key != active_keys[finger] and (current_time - key_last_seen[finger]) > RELEASE_THRESHOLD:
                print(f"{finger} Released {active_keys[finger]}")
                active_keys[finger] = None
                key_start_time[finger] = 0
        else:
            if detected_key:
                if key_start_time[finger] == 0:
                    key_start_time[finger] = current_time
                elif (current_time - key_start_time[finger]) >= PRESS_THRESHOLD:
                    active_keys[finger] = detected_key
                    print(f"{finger} Pressed {active_keys[finger]}")
            else:
                key_start_time[finger] = 0

    cv2.imshow("Multi-Finger Keyboard", frame)
